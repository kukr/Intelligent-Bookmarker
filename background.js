...
async function getTopicLabel(url) {
  ...
  async function categorizeAndBookmark(tab) {
    ...
    console.log('Putting ' + tab.url + ' in Others folder');
    let folder = await findOrCreateFolder("Others");
    await createBookmark(tab.title, tab.url, folder.id);

    const category = await getTopicLabel(tab.url);

    console.log('Deleting ' + tab.url + ' from Others folder');
    chrome.bookmarks.removeTree(folder.id);
  

    const folderTitle = `${category}`;
    console.log('Putting ' + tab.url + ' in ' + folderTitle + ' folder');
    folder = await findOrCreateFolder(folderTitle);
    await createBookmark(tab.title, tab.url, folder.id);
  }
